...
class InspirerScreen extends Component {  
  ...
  async componentDidMount() {
    this.props.actions.getInspirer({ uuid: this.state.uuid });
  ...
  _renderContent = () => {
    ...
    const isFavourite = this.props.favourites.places[nid];
    const entity = this.props.pois ? this.props.pois[nid] : null;
    if (entity) {
            ...
            <ScrollView style={styles.fill} contentContainerStyle={{paddingBottom: this.props.insets.bottom + 10}}>
                ...
                <View style={[styles.container]}>
                  ...
                  {this.state.relatedPois && (
                    this._renderRelatedList("Luoghi correlati", this.state.relatedPois)
                  )}
...
function InspirerScreenContainer(props) {
  ...
  return <InspirerScreen {...props} navigation={navigation} route={route} store={store} insets={insets} />;
}
...
const mapStateToProps = state => {
  return {
    ...
    pois: state.graphqlState.inspirer,
    poisSuccess: state.graphqlState.inspirerSuccess,
    poisError: state.graphqlState.inspirerError,
    poisLoading: state.graphqlState.inspirerLoading,
