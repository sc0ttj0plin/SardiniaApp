...
class Login extends Component {
  ...
  _onBackPress = () => {
    if (this.state.loginStep === AUTH_STATES.PROFILE_REMOVE)
      ...
      this.state.loginStep === AUTH_STATES.PROFILE_EDIT &&
      !this.props.auth.user.info
    )
      /* user has pressed back without completing profile */
      this._goBackScreen();
    else if (this.state.loginStep === AUTH_STATES.PROFILE_EDIT)
      if (this.props.auth.user.info)
        this.setState({ loginStep: AUTH_STATES.PROFILE_SHOW });
      else this._goBackScreen();
  ...
  _renderProfileEdit = () => {
    ...
    const { usernameError, dateError, countryError, sexError } = this.state;
      ...
      <View style={styles.mainView}>
        <View style={styles.view0}>
          <View style={styles.view1s}>
            ...
            <Form>
              <Item
                style={[styles.item1, usernameError ? styles.itemError : {}]}
                regular
              >
                <Input
                  ...
                  onChangeText={(text) =>
                    this._setUsernameField("username", "usernameError", text)
                  }
                />
              </Item>
              <Item
              ...
              >
                ...
                <Picker
                  ...
                  onValueChange={(value) =>
                    this.setState({ age: value, dateError: false })
                  }
                >
      ...
      return (
        <ConnectedScreenErrorBoundary>
          <View style={[styles.fill, { paddingTop: Layout.statusbarHeight }]}>
            ...
            {loginStep === AUTH_STATES.PROFILE_REMOVE &&
              this._renderProfileRemove()}
            {loginStep === AUTH_STATES.PROFILE_EDIT &&
