...
/**
 * MapViewItinerary is used in ItineraryScreen to show a set of markers in a map once the user has selected 
 * the itinerary of interest. A scroll in the list of pois triggers the map focus to shift to the current marker.
 */
class MapViewItinerary extends PureComponent {
...
function ConnectedMapViewItineraryContainer(props) {
  const navigation = useNavigation();
  const route = useRoute();
  const store = useStore();

  return <MapViewItinerary 
    {...props}
...
})(ConnectedMapViewItineraryContainer)